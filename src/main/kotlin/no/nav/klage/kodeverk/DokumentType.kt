...
enum class DokumentType(
    ...
    FORLENGET_BEHANDLINGSTIDSBREV(
        "9",
        "Forlenget behandlingstidsbrev",
        "Forlenget behandlingstidsbrev",
        "Forlenget behandlingstidsbrev.pdf",
    ),
    EKSPEDISJONSBREV_TIL_TRYGDERETTEN(
        "10",
        "Ekspedisjonsbrev til Trygderetten",
        "Ekspedisjonsbrev til Trygderetten",
        "Ekspedisjonsbrev til Trygderetten.pdf",
    ),
    ;
    ...
    companion object {
        ...
        fun fromBeskrivelse(beskrivelse: String): DokumentType =
            entries.firstOrNull { it.beskrivelse == beskrivelse }
...
class DokumentTypeConverter : AttributeConverter<DokumentType, String?> {
    override fun convertToDatabaseColumn(entity: DokumentType?): String? = entity?.id

    override fun convertToEntityAttribute(id: String?): DokumentType? = id?.let { DokumentType.of(it) }
}
