...
enum class AzureGroup(
    ...
    override fun toString(): String = "AzureGroup(id='$id', navn='$navn', reference='$reference')"
...
class AzureGroupConverter : AttributeConverter<AzureGroup, String?> {
    override fun convertToDatabaseColumn(entity: AzureGroup?): String? = entity?.id

    override fun convertToEntityAttribute(id: String?): AzureGroup? = id?.let { AzureGroup.of(it) }
}
